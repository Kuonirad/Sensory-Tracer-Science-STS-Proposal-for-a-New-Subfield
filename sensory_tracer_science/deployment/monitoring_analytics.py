...
class MonitoringSystem:
    ...
    def _initialize_core_metrics(self):
        ...
        core_metrics = [
            ...
            MonitoringMetric("sts_requests_total", MetricType.COUNTER, "Total HTTP requests", "requests"),
            MonitoringMetric("sts_request_duration", MetricType.HISTOGRAM, "Request duration", "seconds"),
            MonitoringMetric("sts_active_connections", MetricType.GAUGE, "Active connections", "connections"),
            MonitoringMetric("sts_error_rate", MetricType.GAUGE, "Error rate", "percentage"),
            
            # Neural Tracer Metrics
            MonitoringMetric("sts_neural_experiments_total", MetricType.COUNTER, "Neural tracer experiments", "experiments"),
            MonitoringMetric("sts_neural_experiment_duration", MetricType.HISTOGRAM, "Neural experiment duration", "seconds"),
            MonitoringMetric("sts_neural_safety_violations", MetricType.COUNTER, "Neural safety violations", "violations"),
            MonitoringMetric("sts_neural_biocompatibility_score", MetricType.GAUGE, "Biocompatibility score", "score"),
            
            # Quantum Tracer Metrics
            MonitoringMetric("sts_quantum_experiments_total", MetricType.COUNTER, "Quantum tracer experiments", "experiments"),
            MonitoringMetric("sts_quantum_coherence_time", MetricType.GAUGE, "Quantum coherence time", "microseconds"),
            MonitoringMetric("sts_quantum_fidelity", MetricType.GAUGE, "Quantum fidelity", "percentage"),
            MonitoringMetric("sts_quantum_error_rate", MetricType.GAUGE, "Quantum error rate", "percentage"),
            
            # Brillouin Tracer Metrics
            MonitoringMetric("sts_brillouin_measurements_total", MetricType.COUNTER, "Brillouin measurements", "measurements"),
            MonitoringMetric("sts_brillouin_signal_quality", MetricType.GAUGE, "Signal quality", "db"),
            MonitoringMetric("sts_brillouin_optical_power", MetricType.GAUGE, "Optical power", "watts"),
            
            # System Resource Metrics
            MonitoringMetric("sts_cpu_usage", MetricType.GAUGE, "CPU usage", "percentage"),
            MonitoringMetric("sts_memory_usage", MetricType.GAUGE, "Memory usage", "bytes"),
            MonitoringMetric("sts_disk_usage", MetricType.GAUGE, "Disk usage", "bytes"),
            MonitoringMetric("sts_network_io", MetricType.COUNTER, "Network I/O", "bytes"),
            
            # Database Metrics
            MonitoringMetric("sts_db_connections", MetricType.GAUGE, "Database connections", "connections"),
            MonitoringMetric("sts_db_query_duration", MetricType.HISTOGRAM, "Database query duration", "seconds"),
            MonitoringMetric("sts_db_deadlocks", MetricType.COUNTER, "Database deadlocks", "deadlocks"),
            
            # Security Metrics
            MonitoringMetric("sts_authentication_attempts", MetricType.COUNTER, "Authentication attempts", "attempts"),
            MonitoringMetric("sts_authentication_failures", MetricType.COUNTER, "Authentication failures", "failures"),
            MonitoringMetric("sts_security_violations", MetricType.COUNTER, "Security violations", "violations"),
            MonitoringMetric("sts_access_denied", MetricType.COUNTER, "Access denied events", "events"),
        ]
    ...
    def _send_alert_notifications(self, alert_rule: AlertRule, alert_data: Dict[str, Any]):
    ...
    def get_system_health_status(self) -> Dict[str, Any]:
        ...
        if 'sts_error_rate' in self.metrics_storage:
            ...
            health_indicators.append(1.0 if error_rate < 5.0 else 0.5 if error_rate < 10.0 else 0.0)
        
        if 'sts_cpu_usage' in self.metrics_storage:
            cpu_usage = self.metrics_storage['sts_cpu_usage'].value
            health_indicators.append(1.0 if cpu_usage < 70.0 else 0.7 if cpu_usage < 85.0 else 0.3)
        
        if 'sts_memory_usage' in self.metrics_storage:
            memory_usage = self.metrics_storage['sts_memory_usage'].value
            health_indicators.append(1.0 if memory_usage < 75.0 else 0.7 if memory_usage < 90.0 else 0.3)
        
        # Calculate overall health score
        overall_health = sum(health_indicators) / len(health_indicators) if health_indicators else 1.0
    ...
    def _export_prometheus_format(self) -> str:
        ...
        for metric in self.metrics_storage.values():
            ...
            labels_str = ""
            if metric.labels:
                label_pairs = [f'{key}="{value}"' for key, value in metric.labels.items()]
...
def run_monitoring_demo():
    ...
    print(f"   Total requests: {perf_report['application_performance']['total_requests']}")
    ...
    print(f"   Security events: {compliance_report['audit_summary']['security_events']}")
