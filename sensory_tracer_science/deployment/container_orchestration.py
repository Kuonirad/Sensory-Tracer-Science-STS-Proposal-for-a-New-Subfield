...
from .production_config import DeploymentEnvironment, ProductionConfig
...
"""
        ...
        compose_config: Dict[str, Any] = {
            ...
            "services": {
                "sts-app": {
                    ...
                    "ports": [
                        f"{self.container_config.port}:{self.container_config.port}"
                    ],
                    ...
                    "tmpfs": (
                        ["/tmp", "/var/tmp"]
                        if self.container_config.read_only_root_filesystem
                        else None
                    ),
                },
                "postgres": {
                ...
                "redis": {
                    ...
                    "command": [
                        ...
                        "${STS_REDIS_PASSWORD}",
                        "--maxmemory",
                        f"{self.production_config.redis.max_memory_mb}mb",
                        "--maxmemory-policy",
                        self.production_config.redis.max_memory_policy,
                    ],
            ...
            "networks": {"sts-network": {"driver": "bridge"}},
        }
    ...
    def generate_kubernetes_deployment(self) -> Dict[str, Any]:
        ...
        deployment = {
            ...
            "spec": {
                ...
                "selector": {
                    "matchLabels": {"app": "sts-framework", "component": "api"}
                },
                "template": {
                    "metadata": {
                        "labels": {
                    ...
                    "spec": {
                        ...
                        "volumes": [
                            ...
                            {
                                "name": "data-volume",
                                "persistentVolumeClaim": {"claimName": "sts-data-pvc"},
                            },
                        ],
                        "affinity": {
                            "podAntiAffinity": {
    ...
    def generate_kubernetes_service(self) -> Dict[str, Any]:
        ...
        service: Dict[str, Any] = {
            ...
            "metadata": {
                ...
                "namespace": self.kubernetes_config.namespace,
                "labels": {"app": "sts-framework", "component": "api"},
                "annotations": {
                    "service.beta.kubernetes.io/aws-load-balancer-type": (
                        "nlb"
                        if self.kubernetes_config.service_type == "LoadBalancer"
                        else None
                    )
                },
            },
            "spec": {
                ...
                "selector": {"app": "sts-framework", "component": "api"},
                "ports": [
                    {
                        "name": "http",
                        "port": 80,
    ...
    def generate_kubernetes_hpa(self) -> Dict[str, Any]:
        ...
        hpa = {
            ...
            "metadata": {
                ...
                "namespace": self.kubernetes_config.namespace,
                "labels": {"app": "sts-framework", "component": "autoscaler"},
            },
            "spec": {
                "scaleTargetRef": {
                    "apiVersion": "apps/v1",
    ...
    def generate_kubernetes_ingress(self) -> Dict[str, Any]:
        ...
        ingress: Dict[str, Any] = {
            ...
            "metadata": {
                ...
                "namespace": self.kubernetes_config.namespace,
                "labels": {"app": "sts-framework", "component": "ingress"},
                "annotations": {
                    ...
                    "nginx.ingress.kubernetes.io/rate-limit": str(
                        self.production_config.security.rate_limit_requests_per_minute
                    ),
        ...
        if self.kubernetes_config.tls_enabled:
            ingress["spec"]["tls"] = [
                {
                    "hosts": [
                        f"sts-api-{self.production_config.environment.value}.company.com"
                    ],
    ...
    def build_container_image(self, push: bool = False) -> str:
        ...
        print(
            f"🔨 Building container image: {self.container_config.image_name}:{self.container_config.image_tag}"
        )
        ...
        try:
            ...
            image_tag = (
                f"{self.container_config.image_name}:{self.container_config.image_tag}"
            )
    ...
    def deploy_to_kubernetes(self, manifests_dir: str = "k8s") -> List[str]:
        ...
        print(
            f"🎉 Deployment completed: {len(deployed_resources)}/{len(manifests)} resources deployed"
        )
    ...
    def generate_deployment_package(self, output_dir: str = "deployment") -> str:
        ...
        print(
            f"📦 Generating deployment package for {self.production_config.environment.value}"
        )
...
"""
        ...
        print(
            f"   Files: Dockerfile, docker-compose.yml, kubernetes/, scripts/, README.md"
        )
...
def create_deployment_packages() -> None:
    ...
    for env in environments:
        ...
        package_path = orchestrator.generate_deployment_package(
            f"deployment-{env.value}"
        )
