...
class BrillouinScatteringParameters:
    ...
    nonlinear_threshold_power: float = (
        1e-3  # W (threshold for significant nonlinear effects)
    )
...
class FiberOpticBrillouinTracer:
    ...
    def __init__(
        self,
        fiber_length: float,
        parameters: Optional[BrillouinScatteringParameters] = None,
    ):
        """
        ...
        if light_speed_in_fiber > max_speed:
            raise ValueError(
                f"Light speed in fiber {light_speed_in_fiber:.2e} m/s "
    ...
    def brillouin_frequency_shift(
        self, temperature: float = 300.0, strain: float = 0.0
    ) -> float:
        """
    ...
    def brillouin_power_coupling(
        self, pump_power: float, distance: float
    ) -> Tuple[float, float]:
        """
        ...
        attenuated_pump_power = pump_power * np.exp(
            -self.params.linear_attenuation * distance
        )

        # Brillouin gain (three-wave coupling)
        gain_factor = (
            self.params.brillouin_gain_coefficient
            * attenuated_pump_power
            * distance
            / self.params.effective_area
        )
    ...
    def propagation_simulation(
        ...
        if input_energy > self.max_input_energy:
            raise ValueError(
                f"Input energy {input_energy:.2e} J exceeds safe limit "
        ...
        for i, z in enumerate(z_points):
            ...
            brillouin_shift[i] = self.brillouin_frequency_shift(
                local_temp, local_strain
            )

            # Power evolution due to Brillouin scattering
            local_stokes, local_acoustic = self.brillouin_power_coupling(
                current_pump_power, dz
            )
            ...
            total_energy[i] = (
                current_pump_power + local_stokes + local_acoustic
            ) * pulse_width
    ...
    def extract_sensing_information(
        ...
        extracted_temp = (
            300.0
            + (brillouin_shift - self.params.brillouin_frequency_shift)
            / temp_coefficient
        )
    ...
    def validate_tracer_operation(
        self,
        input_energy: float,
        simulation_results: Dict[str, np.ndarray],
        sensing_info: Dict[str, Any],
...
class BrillouinTracerExperiment:
    ...
    def create_test_environment(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        temperature = (
            300.0  # Room temperature baseline
            + 5.0 * np.sin(2 * np.pi * position / 100.0)  # Periodic variation
            + 10.0 * np.exp(-(((position - 500.0) / 50.0) ** 2))
        )  # Hotspot at 500m

        # Strain profile: linearly increasing + localized strain concentration
        strain = 1e-6 * position / 1000.0 + 50e-6 * np.exp(  # Gradient
            -(((position - 750.0) / 25.0) ** 2)
    ...
    def run_brillouin_test(self, input_energy: float = 1e-9) -> Dict[str, Any]:
        ...
        """
        print(
            f"Running Brillouin Tracer Test with {input_energy:.2e} J input energy..."
        )
        ...
        is_valid, status_message = self.tracer.validator.system_status(
            validation_results
        )
        ...
        test_results = {
            ...
            "energy_conservation_error": validation_results[
                "energy_audit"
            ].error_magnitude,
            "information_loss_percent": validation_results[
                "information_balance"
            ].error_magnitude
            * 100,
    ...
    def generate_test_report(self, test_results: Dict[str, Any]) -> str:
        ...
        report += (
            f"  Information Loss: {test_results['information_loss_percent']:.2f}%\n"
        )
        ...
        report += (
            f"  Total Information: {sensing['total_information_capacity']:.1f} bits\n"
        )
...
def run_comprehensive_brillouin_tests() -> Dict[str, Any]:
    ...
    try:
        ...
        results["energy_limit_enforcement"] = (
            "FAILED - should have rejected high energy"
        )
...
if __name__ == "__main__":
    ...
    if "standard_operation" in test_results:
        ...
        detailed_report = experiment.generate_test_report(
            test_results["standard_operation"]
        )
        print(f"\n{detailed_report}")
