...
class STSPhysics:
    ...
    def debye_length(ionic_strength: float, temperature: float = 310.0) -> float:
        ...
        """
        import math
        return math.sqrt((EPSILON_0 * K_B * temperature) / 
                        (2 * E_CHARGE**2 * ionic_strength * N_A))
    
    @staticmethod
    def nernst_potential(concentration_in: float, concentration_out: float, 
                        valence: int = 1, temperature: float = 310.0) -> float:
        """
        ...
        """
        import math
        return (R_GAS * temperature) / (valence * F_FARADAY) * \
               math.log(concentration_out / concentration_in)
    
    @staticmethod
    def osmotic_pressure(concentration_diff: float, temperature: float = 310.0) -> float:
        """
    ...
    @staticmethod
    def two_photon_excitation_rate(power_avg: float, pulse_duration: float, 
                                 rep_rate: float, wavelength: float = 810e-9) -> float:
        """
    ...
    @staticmethod
    def photobleaching_rate(wavelength: float, intensity: float, 
                          temperature: float = 310.0) -> float:
        """
        ...
        temperature_factor = math.exp(-1000 * (1/temperature - 1/ref_temperature) / R_GAS)
...
def validate_augmented_physics() -> Dict[str, Any]:
    ...
    nernst_ca = STSPhysics.nernst_potential(100e-9, 2e-3, 2, 310.0)  # typical Ca²⁺ gradient
    results['nernst_potential_ca'] = nernst_ca
    assert 0.1 < nernst_ca < 0.15, f"Ca²⁺ Nernst potential unrealistic: {nernst_ca:.3f} V"
    ...
    assert 1e3 < excitation_rate < 1e7, f"Excitation rate unrealistic: {excitation_rate:.1e} photons/s"
    ...
    assert abs(bleach_rate - K_BLEACH) < 1e-4, \
...
def validate_physical_consistency() -> Dict[str, Any]:
    ...
    results = {
        ...
        'debye_consistency': abs(STSPhysics.debye_length(0.15) - LAMBDA_DEBYE_150mM) / LAMBDA_DEBYE_150mM < 5.0,
    ...
    assert results['two_photon_cross_section_reasonable'], "Two-photon cross-section unrealistic"
...
def validate_uncertainty_propagation() -> Dict[str, Any]:
    ...
    logbb_samples = np.random.normal(UncertaintyParameters.LOGBB_MEAN, 
                                    UncertaintyParameters.LOGBB_STD, n_samples)
    koff_samples = np.random.normal(UncertaintyParameters.K_OFF_MEAN,
                                   UncertaintyParameters.K_OFF_STD, n_samples)
    
    results = {
        'logbb_mean_error': abs(np.mean(logbb_samples) - UncertaintyParameters.LOGBB_MEAN),
        'logbb_std_error': abs(np.std(logbb_samples) - UncertaintyParameters.LOGBB_STD),
        'koff_mean_error': abs(np.mean(koff_samples) - UncertaintyParameters.K_OFF_MEAN),
