...
def run_biocompatible_test(
    tissue_size: str = "standard", duration: float = 60.0, verbose: bool = False
) -> Dict[str, Any]:
    ...
    results = experiment.run_neural_tracer_test(
        simulation_time=duration, dt=min(duration / 10, 10.0)
    )
...
def run_quantum_test(
    measurement_type: str = "standard", precision: str = "normal", verbose: bool = False
) -> Dict[str, Any]:
    ...
    results = experiment.run_quantum_sensing_experiment(
        sensing_parameters=sensing_params, measurement_duration=params.measurement_time
    )
    execution_time = time.time() - start_time
...
def run_brillouin_test(
    fiber_length: float = 1000.0, input_energy: float = 1e-9, verbose: bool = False
) -> Dict[str, Any]:
...
def run_performance_benchmark(
    test_type: str = "all", iterations: int = 3
) -> Dict[str, Any]:
    ...
    if test_type in ["all", "biocompatible"]:
        ...
        for i in range(iterations):
            result = run_biocompatible_test(
                tissue_size="standard", duration=30.0, verbose=False
            )
...
Examples:
  ...
  sts-test --benchmark --iterations 5
    ...
    test_group.add_argument(
        "--biocompatible",
        action="store_true",
        help="Run biocompatible neural tracer test",
    )
    test_group.add_argument(
        "--quantum", action="store_true", help="Run quantum enhanced tracer test"
    )
    test_group.add_argument(
        "--brillouin", action="store_true", help="Run fiber optic Brillouin tracer test"
    )
    test_group.add_argument(
    ...
    parser.add_argument(
        "--measurement-type", default="standard", help="Type of quantum measurement"
    )
    ...
    parser.add_argument(
        "--benchmark-type",
        choices=["all", "biocompatible", "quantum", "brillouin"],
        default="all",
    ...
    try:
        ...
        if args.biocompatible:
            ...
            result = run_performance_benchmark(
                test_type=args.benchmark_type, iterations=args.iterations
            )
